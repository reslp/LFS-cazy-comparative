...
###till here
Greens <- function(n) {
  cols<-colorRampPalette(brewer.pal(9,"Greens"))(n)
  cols[1:n]
}
# get colors
...
labels <- rev(tree$tip.label)

data <- read.csv(countsfile, sep="\t")
data[is.na(data)] <- 0
data$Total <-NULL
# remove peroxi species used for clustering orthologs
data$Characterized_transporters <- NULL
data$Orthogroup <- gsub(";", "__", data$Orthogroup)
rownames(data) <- make.names(data$Orthogroup, unique=TRUE)

# get rid of extram columns
data$Orthogroup <- NULL
data$Orthogroup_old <- NULL
data <- t(as.matrix(data))

#limit to only tip labels (get rid of extra species)
data <- data[match(labels, rownames(data)), ]
...
pdf(file=paste0(wd,"/pod_heatmap.pdf"), width=11.7, height=8.3)
print(hm2)
dev.off()

# this plot is not needed
#pdf(file=paste0(wd,"/pod_heatmap_combined.pdf"), width=11.7, height=8.3)
#print(treep+hm)
#dev.off()
