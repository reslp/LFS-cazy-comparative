...
########################################################################################
# modified by Philipp Resl to work with Python3 and to output the results table
...
tablefilename = outprefix + "_table.txt";
ltreefilename = outprefix + "_cafe_labeled_tree.tre";
...
tablefile.write("\tExpansions\tGenes Gained\tEqual\tContractions\tGenes Lost\tFamilies Lost\tAverage Expansion\tSig Expansions\tSig Contractions\tTotal Sig Changes\n");
...
for species in results_main:
	...
	tablefile.write(outline.strip("\t")+"\n")
	print (outline);
tablefile.close()
print
ltreefile = open(ltreefilename, "w")
ltreefile.write(labeled_tree +";\n")
ltreefile.close()
